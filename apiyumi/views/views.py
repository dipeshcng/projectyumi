...
class BusinessProfileAPIView(APIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        qset = BusinessDetail.objects.get(id=item)
        serializer = BusinessProfileSerializer(
            qset, data=request.data, partial=True)
...
class GraduateProfileAPIView(APIView):
    ...
    def get(self, request):
        ...
        serializer = GraduatesDetailSerializer(
            grad, context={'request': request})
        resp = {
            'status': status.HTTP_200_OK,
            'message': 'success',
            'data': serializer.data
...
class EventListAPIView(APIView):
    permission_classes = [VolunteerOnlyPermission | GraduateOnlyPermission |
                          SuperAdminOnlyPermission | AdminOnlyPermission | BusinessOnlyPermission]
    ...
    def get(self, request):
        try:
            ...
            if hasattr(usr, 'volunteer') | hasattr(usr, 'graduate'):
                event_list = Event.objects.all().order_by('-created_at').filter(status='Active',
                                                                                event_post_end_date__gte=str(date.today()))
            else:
        ...
        try:
            ...
            if hasattr(user, 'volunteer') or hasattr(user, 'graduate') or hasattr(user, 'hostbusiness'):
                ...
                if event.status == "Active" and event.event_post_end_date >= date.today():
                    serializer = EventDetailSerialzer(
                ...
                event = Event.objects.get(id=pk)
                serializer = EventDetailForAdminSerialzer(
                    event, context={'request': request})
                res = {
                    'status': status.HTTP_200_OK,
                    'message': 'success',
...
class RegisterUnregisterForEventAPIView(APIView):
    permission_classes = [VolunteerOnlyPermission |
                          GraduateOnlyPermission | BusinessOnlyPermission]
...
class JobCreateUpdateAPIView(APIView):
    permission_classes = [AdminOnlyPermission |
                          SuperAdminOnlyPermission | BusinessOnlyPermission]
    ...
    def patch(self, request, pk=None):
        ...
        if user.is_superuser == True or hasattr(user, 'admin'):
            serializer = JobUpdateForAdminSerializer(
...
class JobListAPIView(APIView):
    permission_classes = [GraduateOnlyPermission | AdminOnlyPermission |
                          SuperAdminOnlyPermission | BusinessOnlyPermission]
    pagination_class = CustomPageNumberPagination
    ...
    def get(self, request):
        try:
            ...
            if hasattr(user, 'graduate'):
                job_qs = Job.objects.filter(status="Active", application_end_date__gte=str(
                    date.today())).order_by("-created_at")
                result_page = paginator.paginate_queryset(job_qs, request)
                serializer = JobListDetailSerializer(
                    result_page, many=True, context={'request': request})
            elif hasattr(user, 'hostbusiness'):
                job_qs = Job.objects.filter(
                    posted_by=user).order_by("-created_at")
                result_page = paginator.paginate_queryset(job_qs, request)
                serializer = JobListForAdminSerializer(
                    result_page, many=True, context={'request': request})
...
class JobDetailAPIView(APIView):
    permission_classes = [GraduateOnlyPermission | BusinessOnlyPermission |
                          SuperAdminOnlyPermission | AdminOnlyPermission]
        ...
        try:
            ...
            if hasattr(user, 'graduate'):
                job = Job.objects.filter(
                    id=pk, status='Active', application_end_date__gte=str(date.today())).first()
                serializer = JobListDetailSerializer(
                ...
                serializer = JobListDetailForAdminSerializer(
                    job, context={'request': request, 'job': job})
                res = {
                    'status': status.HTTP_200_OK,
                    'data': serializer.data
                }
...
class JobStatusAPIView(APIView):
    ...
    def patch(self, request, pk=None):
        ...
        print(request.data.get('approve'))
...
class ProgramListDetailAPIView(APIView):
    permission_classes = [GraduateOnlyPermission | VolunteerOnlyPermission |
                          AdminOnlyPermission | SuperAdminOnlyPermission]
    pagination_class = CustomPageNumberPagination
            ...
            if pk is None:
                ...
                if hasattr(user, 'volunteer'):
                    serializer = ProgramListSerializer(
                        program, context={'request': request})
                    data = serializer.data
                    res = {
                        'status': status.HTTP_200_OK,
                        'data': data
...
class ProgramRegisterAPIView(APIView):
    permission_classes = [GraduateOnlyPermission |
                          VolunteerOnlyPermission | BusinessOnlyPermission]
...
class ProgramDocumentCreateDeleteAPIView(APIView):
    ...
    def post(self, request, program_id=None):
        ...
        serializer = ProgramDocumentSerializer(data=request.FILES, context={
